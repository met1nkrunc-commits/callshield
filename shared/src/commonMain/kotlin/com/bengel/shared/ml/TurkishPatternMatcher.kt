...
class TurkishPatternMatcher {
    ...
    fun analyze(text: String, sender: String): PatternMatchResult {
        ...
        val investmentMatches = TurkishFraudConstants.INVESTMENT_SCAM_KEYWORDS
            .filter { normalized.contains(it.normalizeForMatching()) }
        val edevletMatches    = TurkishFraudConstants.EDEVLET_PHISHING_KEYWORDS
            .filter { normalized.contains(it.normalizeForMatching()) }
        val cargoMatches      = TurkishFraudConstants.CARGO_SCAM_KEYWORDS
            .filter { normalized.contains(it.normalizeForMatching()) }
        val bankMatches       = TurkishFraudConstants.BANK_PHISHING_KEYWORDS
            .filter { normalized.contains(it.normalizeForMatching()) }
        ...
        matchedPatterns.addAll(edevletMatches)
        matchedPatterns.addAll(cargoMatches)
        matchedPatterns.addAll(bankMatches)
        ...
        val hasPhishingUrl =
            ...
            TurkishFraudConstants.EDEVLET_DOMAIN_PATTERNS.any { it.containsMatchIn(normalized) }
        ...
        val urlAlone = hasPhishingUrl &&
                ...
                investmentMatches.isEmpty() && edevletMatches.isEmpty() &&
                cargoMatches.isEmpty() && bankMatches.isEmpty() && !hasBtkCode

        val riskLevel = when {
            hasBtkCode                                                             -> RiskLevel.BLOCKED
            senderIsPhoneNumber && (phishingMatches.isNotEmpty()
                    || edevletMatches.isNotEmpty() || bankMatches.isNotEmpty())   -> RiskLevel.BLOCKED
            bettingScore >= 6                                                      -> RiskLevel.BLOCKED
            investmentScore >= 6                                                   -> RiskLevel.BLOCKED
            edevletMatches.isNotEmpty() || bankMatches.isNotEmpty()               -> RiskLevel.HIGH
            legalMatches.isNotEmpty() || phishingMatches.isNotEmpty() || urlAlone -> RiskLevel.HIGH
            hasPhishingUrl                                                         -> RiskLevel.HIGH
            cargoMatches.isNotEmpty()                                             -> RiskLevel.HIGH
            bettingScore >= 1 || investmentScore >= 1
                    || socialMatches.isNotEmpty()                                 -> RiskLevel.HIGH
